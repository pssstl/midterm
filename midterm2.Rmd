...
df_original <- janitor::clean_names(original) %>% 
...
df_cumulative <- df_original %>% group_by(academic_year, school, student_group, grade_level) %>% 
  mutate(cn = cumsum(n))
...
df_student_group <- df_cumulative %>%
  select(-n) %>% 
...
df_estimate_district <- df_student_group %>% 
  group_by(academic_year, grade_level, school, district) %>% 
  nest() %>% 
  mutate(es = map_dbl(data, ~ gap(.x, "white", "hispanic_latino")))

df_estimate_district
```
